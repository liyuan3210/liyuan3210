...
4．**私有ip地址(只能在局域内存在的地址,后面子网划分参照这个来做)**：
...
​		局域网内理论上可以按照B类地址划分188.188.1.1来划分子网络,也可以通,但不满足ICANN分配规范.会出现IP冲突问题.
...
**主机位全为0:表示网络号（一般配网关）** ,一般指路有，防火墙，网关设备
...
**默认网关 (非必填) 当要连接非我所在网段跨网络时才要填　192.168.0.1** ，（填所在网络的**网络号**）
